...
public class SkillNode : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, ISelectHandler, IDeselectHandler
{
    ...
    void ApplyEffect()
    {
        ...
        switch (effectType)
        {
            ...
            case SkillEffectType.IncreasePopulation25: Guardado.instance.AddPopulationBonus(7f); Guardado.instance.SaveData(); break;
    ...
    string GetPreviewValues()
    {
        ...
        switch (effectType)
        {
            ...
            // -------------------------
            // CAPTURA POR FASE (RESTA FIJA DE SEGUNDOS)
            // -------------------------
            ...
            case SkillEffectType.InfectSpeedPhase0_50:
                {
                    ...
                    float tiempoNuevo = Mathf.Max(0.2f, tiempoBase - descuentoNuevo);
            ...
            case SkillEffectType.SpawnBaseFigureOnMaxPhase20:
                {
                    ...
                    float despues = (g.spawnBaseOnMaxPhaseChance + 0.05f) * 100f;

                    if (comprado)
                        sb.AppendLine($"{actual:F0}%");
                    else
                        sb.AppendLine($"{actual:F0}% → {despues:F0}%");
                    break;
                }
